...
use crate::{AccountError, HashLen};
...
impl AccountCheck for SignerAccount {
    fn check(account: &AccountInfo) -> Result<(), ProgramError> {
        if !account.is_signer() {
            return Err(ProgramError::from(AccountError::NotSigner));
        }
        Ok(())
    }
}
...
const TOKEN_2022_ACCOUNT_DISCRIMINATOR_OFFSET: usize = 165; //one byte larger than the legacy TokenAccount to avoid conflict
...
impl AccountCheck for MintInterface {
    fn check(account: &AccountInfo) -> Result<(), ProgramError> {
        if !account.is_owned_by(&TOKEN_2022_PROGRAM_ID) {
            ...
            if data.len().le(&pinocchio_token::state::Mint::LEN) {
                if data.len().le(&TOKEN_2022_ACCOUNT_DISCRIMINATOR_OFFSET) {
                    return Err(ProgramError::InvalidAccountData);
                }
                if data[TOKEN_2022_ACCOUNT_DISCRIMINATOR_OFFSET].ne(&TOKEN_2022_MINT_DISCRIMINATOR)
...
impl AssociatedTokenAccountCheck for AssociatedTokenAccount {
    fn check(
        ...
        .ne(account.key())
        {
            return Err(ProgramError::from(AccountError::InvalidAddress));
        }
...
impl AssociatedTokenAccountInit for AssociatedTokenAccount {
    ...
    fn init_if_needed(
        ...
        payer: &AccountInfo,
        owner: &AccountInfo,
        system_progra: &AccountInfo,
        token_program: &AccountInfo,
    ) -> ProgramResult {
        match Self::check(account, owner, mint, token_program) {
            Ok(_) => Ok(()),
            Err(_) => Self::init(account, mint, payer, owner, system_progra, token_program),
        }
    }
}
...
pub trait ProgramAccountCheck {
    fn check<T: Sized + HashLen>(account: &AccountInfo) -> Result<(), ProgramError>;
}
